...
const PROXY_BASE = 'https://futureloop-og-proxy.vercel.app/api/share';
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  const title = escapeHtml(article.title || 'EU-Posten');
  const description = escapeHtml((article.deck || article.content || '').slice(0, 200));
  const articleUrl = SITE_URL + '/eu-posten?article=' + article.id;
  const proxyUrl = PROXY_BASE + '?id=' + article.id;
  ...
  const html = '<!DOCTYPE html>'
    ...
    + '<script>try{window.location.replace("' + articleUrl.replace(/"/g, '\\"') + '")}catch(e){window.location.href="' + articleUrl.replace(/"/g, '\\"') + '"}</script>'
    + '</body></html>';
